...
const numWorkers = process.env.NUM_WORKERS ? parseInt(process.env.NUM_WORKERS) : 2;
    ...
    for (let i = 0; i < numWorkers; i++) {
      ...
      worker.on('message', (message) => {
        if (message.type === 'saveTriedSet') {
          ...
          saveFoundWallet(message.pubkey, message.seedPhrase, message.balance, message.workerIconst fs = require('fs');
const readline = require('readline');
const { Keypair, Connection, clusterApiUrl } = require('@solana/web3.js');
const { derivePath } = require('ed25519-hd-key');
const bip39 = require('bip39');
