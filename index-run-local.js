...
const foundFile = 'found.json';

if (isMainThread) {
  const seeds = fs.readFileSync('seed-key-words.txt', 'utf8').split('\n');
  ...
  function saveFoundWallet(pubkey, seed, balance, workerId) {
    ...
    let foundWallets = fs.existsSync(foundFile) ? JSON.parse(fs.readFileSync(foundFile, 'utf8')) : [];
